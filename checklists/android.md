...
### AND-FORM-01 Android Auto / Wear OS / TV form-factor reviews
- Basis: opting into a form factor (or a library merging a `CarAppService`) starts a **separate quality review** with its own rejections: "Auto App Quality Guidelines: the content of your application is not able to load", "Parking and Charging Functionality — The app didn't provide meaningful functionality in its category relevant to driving", "Missing ongoing activity: When a user has an ongoing activity…", "non-standalone Wear app can't connect", "App doesn't mention Wear OS in store listing", TV "vertical letterboxing"
- Applies: any app whose manifest or Console opts into Auto/Wear/TV/XR — including phantom services from libraries (CarPlay/Media3 wrappers)
- Verify: strip unintended `CarAppService` / Wear entries with `tools:node="remove"`; if intended, meet the form-factor quality guidelines (ongoing activity on Wear, category functionality on Auto, landscape/leanback on TV) and keep the listing text/screenshots complete for that form factor.
- Cases: [Play 3rd pass](../rejections/community-cases.md#google-play--3rd-pass-2026-08-28-thin-areas--child-safety-declarations-health-connect-form-factors-enforcement)
...
- Play 3rd-pass cases — the SMS "cross-device synchronization" use case means **mirroring**, not forwarding to another number/e-mail (rejected as not matching); core-function proof for SMS/Call Log is re-verified on every update ("couldn't find evidence that the app offers the declared core functionality"). [cases](../rejections/community-cases.md#google-play--3rd-pass-2026-08-28-thin-areas--child-safety-declarations-health-connect-form-factors-enforcement)
...
- Play 3rd-pass cases — Google re-tests **after launch** with the same Sign-in details: a demo account disabled post-launch caused a violation weeks later; paywall-only apps need a subscribed test Google account or a deep-link/licence-testing bypass ("Paywall restriction… Update your app login credentials in the app access declaration"); OTP apps need a fixed number + always-valid code; hardware-bound apps a review mode or public video/guide link. [cases](../rejections/community-cases.md#google-play--3rd-pass-2026-08-28-thin-areas--child-safety-declarations-health-connect-form-factors-enforcement)
...
- Play 3rd-pass cases — Health apps declaration: incidental health features (HIV-status field in a dating app, a breathing exercise, a skincare tracker, a vet portal) must be declared under **"Others"** or removed, otherwise "Inaccurate Health Apps Declaration"; Health Connect "Minimum Scope" rejections list the whole permission group — add a paragraph in the store description naming each data type and the feature it powers, remove Apple/other-platform wording, and note the description reviewed is the one attached to the reviewed release; personal-loan apps: partner bank names/URLs in the description must match the Financial features declaration, and the loan policy forbids storage permissions. [cases](../rejections/community-cases.md#google-play--3rd-pass-2026-08-28-thin-areas--child-safety-declarations-health-connect-form-factors-enforcement)
...
- Play 3rd-pass cases — "The published standards are invalid" triad: the child-safety standards URL must (a) return 200 without login/JS, (b) contain CSAE/child-safety wording, (c) contain the exact app **or** developer name from the listing; social apps also need a named contact, an in-app report path and a CSAM-handling statement; re-verified on every resubmission and appeal — a standards page that silently disappeared led to a "Repeated app rejections" suspension. [cases](../rejections/community-cases.md#google-play--3rd-pass-2026-08-28-thin-areas--child-safety-declarations-health-connect-form-factors-enforcement)
...
- Play 3rd-pass cases — **pausing is not enough**: bundles in paused test tracks and <100 % production rollouts are still what the reviewer sees (symptoms: a months-old screenshot, unused reviewer credentials, a rejection quoting a removed screen); fully replace or deactivate the track; a re-scan of an old bundle re-suspended an app whose appeal had been accepted; "changing anything on the console will reset the review timer". [cases](../rejections/community-cases.md#google-play--3rd-pass-2026-08-28-thin-areas--child-safety-declarations-health-connect-form-factors-enforcement)
- International 3rd-pass step: when Google's template demands "deactivate the non-compliant App Bundle Version… Track: Production and increment the version code" and the Console won't let you, **unpublish the app yourself** (Release → Setup → Advanced settings → App availability) and re-upload to *all* tracks at 100 % — this is what finally cleared it. [cases](../rejections/community-cases.md#cases-from-russian-spanish-portuguese-vietnamese-indonesian-turkish-german-and-french-sources-3rd-pass-2026-08-28)
...
- International 3rd-pass rule: **self-created tester Google accounts** to reach 12 testers led to terminations "related to bot accounts"; friends who install without using don't count — recruit real testers (15–25, 17+ days), keep logs/videos. [cases](../rejections/community-cases.md#cases-from-russian-spanish-portuguese-vietnamese-indonesian-turkish-german-and-french-sources-3rd-pass-2026-08-28)
...
- International 3rd-pass cases: an account that ever monetised must publish a physical address/phone (Authenticator Pro left Play over it); dormant accounts (6 years inactive) get purged with the fee refunded — decide org vs individual before the first paid app and keep at least one active app per year. [cases](../rejections/community-cases.md#cases-from-russian-spanish-portuguese-vietnamese-indonesian-turkish-german-and-french-sources-3rd-pass-2026-08-28)
...
- Play 3rd-pass cases — the organization requirement is triggered by **purpose keywords** in the listing ("invoices, costs", "accounts, bank, savings", clinic contact, classifieds with paid membership, gig apps with coins), not only by declarations; a listing created on a personal account keeps that state after conversion — unpublish and recreate it under the organization account. [cases](../rejections/community-cases.md#google-play--3rd-pass-2026-08-28-thin-areas--child-safety-declarations-health-connect-form-factors-enforcement)
...
- Play 3rd-pass cases — the counter is real: 3–4 rejections on one app within ~30–60 days trigger an Enforcement Process suspension even when each was minor and fixed; after suspension the App access declaration can't be edited and the single appeal is judged on the *original* rejections; association vectors seen in 2026: contractor/agency console access, a shared device or lent laptop, a home network, a D-U-N-S sync delay, transferring your own app between your own accounts; the 180-day rule blocks new evidence. [cases](../rejections/community-cases.md#google-play--3rd-pass-2026-08-28-thin-areas--child-safety-declarations-health-connect-form-factors-enforcement)
- International 3rd-pass rule: **package names and ContentProvider authorities of a terminated account are reserved forever** ("já existe no Google Play") — after termination/re-registration rebuild with a new applicationId and new provider authorities. [cases](../rejections/community-cases.md#cases-from-russian-spanish-portuguese-vietnamese-indonesian-turkish-german-and-french-sources-3rd-pass-2026-08-28)
